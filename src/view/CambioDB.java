...
public class CambioDB extends javax.swing.JDialog {
    ...
    public CambioDB(java.awt.Frame parent, boolean modal) {
        ...
        for (String dato : datos) {
            if(dato!=null&&!dato.equals("mysql")&&!dato.equals("phpmyadmin")&&!dato.equals("postgres")
               &&!dato.equals("template0")&&!dato.equals("template1")){
                cbdb.addItem(dato);
            }
        }
        cbdb.setSelectedItem(Validar.nombreBdUser());
    ...
    private void baceptarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_baceptarActionPerformed
        ...
        if(confirmDialog.response("El control de sus datos es muy importante, asegúrese de seleccionar la opción correcta<br><br>¿Está seguro que desea usar la base de datos seleccionada?", 2)==1){
            cambiarNombreDB();
    ...
    private void cambiarNombreDB(){
        ...
        try (FileWriter escribir = new FileWriter(ruta+"user.dat")) {
            ...
            guardar.println("dbengine="+config.getDbengine());
            guardar.println("driver="+config.getDriver());
            guardar.println("url="+config.getUrl());
            guardar.println("dbname="+cbdb.getSelectedItem().toString());
            guardar.println("dbuser="+config.getDbuser());
            guardar.println("password="+config.getPassword());
        ...
        try(FileWriter escribirDB = new FileWriter(ruta+"database.dat")){
            ...
            guardarDB.println("password="+config.getPassword());
        }catch(IOException ex){
            messageToast.response("El sistema no puede encontrar el archivo de configuración de datos", 250, 80, "danger");
        }
    }
