...
public class ProductoMapper {
    ...
    public void insertarPresentacion(String presentacion){
        try {
            st = cn.createStatement();
            st.executeUpdate("INSERT INTO t_presentacion (descripcion) VALUES('"+presentacion+"');");
        } catch (SQLException ex) {
    ...
    public void mostrarProducto(DefaultTableModel modelo, String nompreProducto, String presentacion){
        ...
        try {
            st = cn.createStatement();
            String complement = "to_char(pf.fecha_vencimiento, 'dd/mm/YYYY')";
            if(Validar.dbengine().equals("mysql")){complement = "DATE_FORMAT(pf.fecha_vencimiento,'%d/%m/%Y')";}
            String query = "SELECT pr.descripcion, p.nombre, p.laboratorio, "+complement+", p.precio_venta_unitario, pf.stock_actual, p.id_producto, pf.id_producto_fechav "
            ...
            int i=1;
            while(rs.next()){
                modelo.addRow(new String[]{""+(i++),rs.getString(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getString(8)});
            }
        } catch (SQLException ex) {
    ...
    public void mostrarControlProducto(DefaultTableModel modelo, String nompreProducto, String presentacion){
        ...
        try {
            ...
            String complement = "to_char(pf.fecha_vencimiento, 'dd/mm/YYYY')";
            if(Validar.dbengine().equals("mysql")){complement = "DATE_FORMAT(pf.fecha_vencimiento,'%d/%m/%Y')";}
            String query = "SELECT tp.descripcion, p.nombre, p.laboratorio, "+complement+", p.precio_venta_unitario, pf.stock_actual, p.id_producto "
    ...
    public CProducto consultarProducto(String idProducto, String fechaVencimiento, int idProductoFechav){
        ...
        try {
            ...
            String complement = "to_char(pf.fecha_vencimiento, 'dd/mm/YYYY')";
            if(Validar.dbengine().equals("mysql")){complement = "DATE_FORMAT(pf.fecha_vencimiento,'%d/%m/%Y')";}
            String query = "SELECT p.id_presentacion, p.id_producto, p.nombre, p.indicacion, p.precio_venta_unitario, p.stock_minimo, "+complement+", pf.stock_actual, "
                   ...
                   .concat("FROM t_producto p INNER JOIN t_producto_fechav pf on pf.id_producto = p.id_producto ")
                   .concat("WHERE p.id_producto = '"+idProducto+"' AND "+complement+"='"+fechaVencimiento+"' ");
            if(idProductoFechav > 0)
                query = query.concat("AND pf.id_producto_fechav = "+idProductoFechav+" ");
            query = query.concat("ORDER BY p.nombre ASC, pf.id_producto ASC;");
            rs = st.executeQuery(query);
            if(rs.next()){
    ...
    public int insertarProductoFechav(CProducto p){
        try {
            st = cn.createStatement();
            return st.executeUpdate("INSERT INTO t_producto_fechav (id_producto,fecha_vencimiento,precio_compra_total,precio_compra_unitario,stock_actual,estado) VALUES('"+p.getIdProducto()+"','"+p.getFechaVencimiento()+"',"+p.getPrecioCompraTotal()+","+p.getPrecioCompraUnitario()+","+p.getStockActual()+",1);");
        } catch (SQLException ex) {
    ...
    public int actualizarProductoFechav(CProducto p){
        try {
            String query = "UPDATE t_producto_fechav SET fecha_vencimiento='"+p.getFechaVencimiento()+"',precio_compra_total="+p.getPrecioCompraTotal()+",precio_compra_unitario="+p.getPrecioCompraUnitario()+",stock_actual="+p.getStockActual()+" WHERE id_producto_fechav="+p.getIdProductoFechav()+";";
            st = cn.createStatement();
